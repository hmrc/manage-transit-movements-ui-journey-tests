...
class ItemDetailsStepDef extends BaseStepDef {
  ...
  And("""^(?:I )?enter (.*) on the 'How many of these are you using\?' page$""") {
    (answer: String) =>
  ...
  And("""^(?:I )?enter (.*) on the 'Enter the additional information' page$""") {
    (answer: String) =>
  ...
  And("""^(?:I )?enter (.+) on the 'Enter your comments for the additional information\' page$""") {
    (answer: String) =>
